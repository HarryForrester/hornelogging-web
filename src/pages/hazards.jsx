    ...
    useEffect(() => {
        const submit = async () => {
            if(confirmationModalState.confirmed) {
                ...
                axios.delete(process.env.REACT_APP_URL+'/deleteHazard', {withCredentials: true, data: { hazardIds: selectedHazardIds } })
                .then(response => {
                    if (response.status === 200) {
                        resetHazard();
                        setHazards(response.data.hazards);
...
//HazardReviewModal
    ...
    const handleReviewSubmit = (comment) => {
     ...
     useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_URL}/hazard`, { withCredentials: true });
                if (response.status === 200) {
    ...
    return (
        <><Container>
            ...
            <Row>
                <Col md={6}>
                    <Form.Check id="selectall" type="checkbox" checked={selectAll} onChange={toggleSelectAll} label="Select All" />
                </Col>
                <Col md={6} className="text-md-end p-3">
                    {selectedHazardIds.length > 0 && (
                        <>
                        <Button variant="outline-secondary" className="selective" style={{ marginRight: '8px' }} onClick={deleteSelected}>
                            <i className="fa fa-trash" aria-hidden="true"></i>&nbsp;Delete
                        </Button>
                        <Button variant="outline-secondary" className="selective" style={{ marginRight: '8px' }} onClick={updateSelected}>
                            <i className="fa fa-magic" aria-hidden="true"></i>&nbsp;Update Review
                        </Button>
                        <Button variant="outline-secondary" className="selective" style={{ marginRight: '8px' }} onClick={editHazard}>
            ...
            {hazards && hazards.filter((hazard) => hazard.searchText.toLowerCase().includes(searchTerm.toLowerCase())
            ).map(hazard => (
                <HazardCard key={hazard._id} hazard={hazard} selectAll={selectedHazardIds.includes(hazard._id)} handleHazardChange={handleHazardChange} />
            ))}
