...
const Library = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:3001/library', { withCredentials: true }); // Replace with your API endpoint
  ...
  const uploadFile = async (file) => {
    try {
        ...
        const response = await axios.post(process.env.REACT_APP_URL + '/library/upload', formData, { withCredentials: true });
          ...
          {files.map(file => (
            <React.Fragment key={file._id}>
              <dt style={{ fontSize: 'medium' }}>
                <form action={`/library/updatefiletype/${file._id}`} method="post">
                  <select
                    ...
                    onChange={() => updateFileType(file._id, document.getElementById('filetype').value)}
                  >
              ...
              <dd>
                ...
                &nbsp;&nbsp;
                <a href={file.uri} download={file.fileName} target="_blank" rel="noopener noreferrer">
                  <FontAwesomeIcon icon={faDownload} />
                </a>
                
                &nbsp;
                <a style={{ marginLeft: 'auto' }} href="#" onClick={() => confirmDelete(file._id)}>
