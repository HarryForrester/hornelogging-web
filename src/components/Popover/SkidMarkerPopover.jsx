...
const SkidMarkerPopover = () => {
    ...
    const handleHazardClick = (hazard) => {
        setSkidModalState((prevState) => ({ ...prevState, hazardModalVisible: true, isSkidModalVisible: false, selectedHazardData: hazard }));
      }
  ...
  const openPdfInNewTab = (item) => {
    ...
    const removeSelectedSkid = async () => {
        ...
        const response = await axios.delete(`http://localhost:3001/pointonmap/${skidMarkerState.selectedMarker._id}/${mapState.currentMapId}`, { withCredentials: true });
    
        if (response.status === 200) {
          setMapState((prevState) => {
            const updatedMarkers = prevState.currentMapMarkers.filter(marker => marker._id !== skidMarkerState.selectedMarker._id);
            return {
              ...prevState,
                    ...
                    {skidMarkerState.selectedMarker.info.selectedDocuments !== null && (
                      <ul className="list-group" style={{ maxHeight: '100px', overflowY: 'auto' }}>
                        {skidMarkerState.selectedMarker.info.selectedDocuments.map((item, index) => (
                          <li key={index} className="list-group-item d-flex justify-content-between align-items-center list-group-item-action" onClick={() => window.open("http://localhost:3001" + item.uri, '_blank')}>
                            {item.fileName}
                          </li>
                        ))}
                      </ul>
