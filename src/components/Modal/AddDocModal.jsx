...
const AddDocModal = ({ docSumbit }) => {
  ...
  */
  const handleClose = () => {
    setSkidModalState((prevState) => ({ ...prevState, isAddDocModalVisible: false, isSkidModalVisible: true }));
  };
  ...
  const handleCheckboxChange = (fileUrl) => {
    ...
    if (fileUrl._id && !skidModalState.selectedDocuments.some((selectedFile) => selectedFile._id === fileUrl._id)) {
  ...
  return (
      <Modal show={skidModalState.isAddDocModalVisible} onHide={handleClose} backdrop="static" >
        ...
        <Modal.Body>
          ...
          <input type="text" className="form-control" size="30" placeholder="Search" id="search-criteria-doc" />
          <br />
          <div className="modal-hazards">
            {mapState.files.map((file) => (
              <div className="card" style={{ marginBottom: '10px' }} key={file.uri}>
                <div className="search-text-doc" style={{ display: 'none' }}>{file.searchText}</div>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={() => docSumbit(skidModalState.selectedDocuments)}>Save changes</Button>
        </Modal.Footer>
      </Modal>
  );
};
